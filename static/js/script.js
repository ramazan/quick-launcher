...
chrome.storage.sync.get(['quickLauncherSitesConfig'], function(config) {
    ...
    if(config['quickLauncherSitesConfig'] && Object.keys(config['quickLauncherSitesConfig']).length > 0){
        initializeCards(config.quickLauncherSitesConfig);
        $("#jsonSiteData").val(JSON.stringify(config.quickLauncherSitesConfig, null, 2));
    }
});
...
$("#saveSiteIcon").click(()=> {
    chrome.storage.sync.set({quickLauncherSitesConfig:JSON.parse($("#jsonSiteData").val())}, function() {
        chrome.storage.sync.get(['quickLauncherSitesConfig'], function(config) {
            if(config['quickLauncherSitesConfig'] && Object.keys(config['quickLauncherSitesConfig']).length > 0){
